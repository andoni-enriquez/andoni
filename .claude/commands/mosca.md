...
- Use `<mark>` tags on supporting beats — pivots, reversals, structural statements that carry the argument forward. Never highlight the alpha (the core thesis, the competitive insight). The reader earns alpha by reading. Use sparingly (see `apps/blog/content/blog/en/inception.mdx` for reference).
